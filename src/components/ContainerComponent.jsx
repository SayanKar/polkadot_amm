...
export default function ContainerComponent(props) {
    ...
    const tabs = [{ "name": "Swap", "icon": <MdSwapVert className="swapIcon" /> }, 
            ...
            <div className="selectTab">
                {
                    tabs.map(tab => {
                        return (<div
                            tabIndex={0}
                            className={"tabStyle " + (activeTab === tab.name ? "activeTab" : "")}
                            onClick={() => changeTab(tab.name)}
                        >
                            <div className={(activeTab === tab.name ? "activeTabIcon" : "tabIcon ")}>{tab.icon}</div>
                            <div className={"tabName " + (activeTab === tab.name ? "activeTabName" : "")}>{tab.name}</div>
                        </div>);
                    })
                }
            </div>
            <div className="centerContainer">
                ...
                {activeTab === "Account" && (
                    <AccountComponent
                        ...
                        holding={{amountOfKAR, amountOfKOTHI, amountOfShare, totalShare, totalKAR, totalKOTHI}}
                    />
                )}
            </div>
        </div>
